...
SYSTEM_PROMPT = (
    ...
    "Tie each lever to a failure signal. If junction scores look weak, broaden or narrow the masking window and, when only a few pairs are suspect, rescore just those junction pairs instead of recomputing everything. If the search cuts off early or collapses to a partial path, change search_mode or choose a different beam_width. If the overlap graph and MLM disagree, switch edge_mode or adjust confirmed_bonus. Do not simply increase beam width monotonically.\n\n"
    f"After each candidate reconstruction, call validity_scorer on that reconstruction. Lower pseudo-perplexity is better, and the run should stop early only if the validity score reaches {cfg['search']['validity_threshold']}. Store the reconstruction, lever choices, and validity score in shared state through the tools. At the end of the allotted iterations, return the best scoring reconstruction found.\n\n"
