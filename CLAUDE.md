...
The task is a **permutation of a fixed fragment set**: every candidate reuses the same fragments, so string composition is invariant and the only thing that varies is ordering. The metric set (revised 2026-07-12) is deliberately small so it doesn't report the same "how many fragments are misordered" signal four times:

- **exact_match** — binary floor (`target == reconstruction`). Rare on this task; a sanity floor, not the headline.
- **similarity** — `difflib.SequenceMatcher` ratio. The single soft string metric kept; `norm_edit_distance` and `lcs_ratio` were **removed** as redundant with it (all three are one underlying ordering signal viewed differently), and edit distance additionally over-penalises a single block swap by ~2×block-length.
- **adjacent_pair_acc** — fraction of true adjacent fragment pairs preserved. Directed and **string-multiset based**, so duplicate/substring-identical fragments don't spuriously read as misordered. Primary ordering metric.
- **longest_correct_run** — longest contiguous correctly-ordered fragment block / n. Credits partial assembly (one long correct chunk vs. scattered correct pairs).
- **kendall_tau** — global ordering correlation (0 = random, 1 = perfect, −1 = reversed). Ranks are matched on fragment strings (`_matched_rank_sequence`) so duplicates don't corrupt it. `fragment_acc` (brittle absolute-position tiling — one early shift cascades) was **removed** in favour of this adjacency/rank view.

Direction: no metric rewards a reversed sequence (kendall → −1, adjacent_pair_acc → 0 on a clean reversal).

**Ground-truth order is not stored** — there is no per-fragment order field in the data. `recover_true_order` re-derives it by greedy longest-first tiling of the target with the fragments (replica 0 tiles the target by construction). When tiling **fails**, the three ordering metrics are `NaN` (not a misleading `0.0`), `compute_all` sets `true_order_recovered=False`, and those samples are dropped from the averages by `nanmean`; the count of usable samples is surfaced in the Cost/Completion section. Predictions are validated as clean permutations via `is_clean_permutation` for the completion count.

**Two things the pipeline assumes are now measured, not taken on faith:**
- **Junction-scorer ranking**, search-independent — `python -m evaluation.junction_ranking` scores a dense junction matrix over all ordered pairs and reports top-1/top-3 successor accuracy and MRR of the true successor. This isolates scorer quality from search dynamics; run it before trusting the scorer.
- **Validity-signal trust** — the in-report Validity Signal Concordance section computes whether lower validity actually corresponds to higher true quality among the candidates each sample tried. A pLM plausibility score rewards *natural-looking* proteins, a poor prior for novel targets, so this concordance is the number to watch, not the validity value itself.
...
- `evaluation/metrics.py` — shared metrics (revised 2026-07-12, see the Metrics section below): `exact_match`, `similarity`, `adjacent_pair_acc`, `longest_correct_run`, `kendall_tau`. Also holds the search-independent `junction_ranking_stats` (used by `evaluation/junction_ranking.py`), the selection-signal trust check `rank_concordance`, the `is_clean_permutation` completion guard, and `nanmean` for NaN-safe averaging.
- `evaluation/junction_ranking.py` — standalone diagnostic (`python -m evaluation.junction_ranking`) that measures how well the raw pLM junction scorer ranks true successor fragments, **independent of search**. This is the only measurement of the pipeline's core assumption; every metric in `metrics.py` is measured after search.
...
The headline of an agentic report is a single three-arm table — **Shuffled Baseline → Deterministic (config defaults) → Agentic Best**, with a `Δ Agentic − Deterministic` column — plus the paired per-sample gain distribution (mean/std/min/max of Agentic − Deterministic) for the significance story (run a Wilcoxon signed-rank test on those per-sample gains). Two further sections are always emitted for agentic runs: **Validity Signal Concordance** (does the validity score used for selection actually track true quality, measured within-sample across iterations via `rank_concordance`; ~0.50 = chance) and **Cost, Efficiency & Completion** (LLM calls/tokens/wall-clock per sample, completion rate, LLM lever-choice failures, and how many samples had a recoverable true order). These come from the `cost_summary` payload attached by `run_agentic`. Reports open with a **"How to Read This Report"** block (`run_type_summary`) that names each column for the active config: the **Deterministic** column is iteration 1 run from `search.default_levers` with no LLM call; the **Agentic Best** column is the iteratively-selected best-validity reconstruction. `first_pass_label()`/`selected_best_label()`/`_iteration1_is_deterministic()` derive the labels and are mode-aware (react mode always drives iteration 1 through the LLM, so it is never labelled deterministic).
...
- **Metric set overhauled (2026-07-12).** Cut from 7 to 5 quality metrics for a permutation-of-fixed-fragments task. **Removed** `fragment_acc` (brittle absolute-position tiling), `norm_edit_distance` and `lcs_ratio` (both redundant with `similarity`; edit distance also over-penalises block swaps). **Kept** `exact_match`, `similarity`, `adjacent_pair_acc`, `kendall_tau` (the last two hardened against duplicate/substring-identical fragments — string-multiset pairs and string-matched ranks). **Added** `longest_correct_run` (partial-assembly credit). Tiling-failure now yields `NaN` + a `true_order_recovered` flag instead of a silent `0.0`, and averaging is NaN-safe. **Added the two measurements the pipeline previously only assumed:** `evaluation/junction_ranking.py` (search-independent junction-scorer top-1/top-3/MRR) and an in-report Validity Signal Concordance section (does the selection signal track true quality). Agentic reports now also carry a Cost, Efficiency & Completion section (LLM calls/tokens/wall-clock, completion & LLM-failure rates). See the Metrics section. Mixture/multi-proteome separation is **out of scope** — no metric serves it (the `organism: mixture` data path still exists but is not a metrics concern).
